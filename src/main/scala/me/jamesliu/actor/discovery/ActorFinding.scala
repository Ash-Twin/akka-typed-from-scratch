...
import me.jamesliu.actor.discovery.ActorFinding.{Command, Data, XaskSome, XreceivefromY, XreceivefromZ}
import me.jamesliu.actor.discovery.SupplierProtocol.{YreplytoX, ZreplytoX}

import scala.collection.mutable
import scala.concurrent.duration.DurationInt
import scala.util.{Failure, Success}
class ActorFinding(actorContext: ActorContext[Command]) extends AbstractBehavior[Command](actorContext) {
          ...
          case Success(value)     =>
            value match {
              case XreceivefromZ(baseFiat, target, z) =>
                ...
                val mapx = Map("key"->"value")
                val mapv = Map(("key","value"))
...
object ActorFinding {
  ...
  def apply(): Behavior[Command] = Behaviors.setup[Command] { ctx =>
    ctx.system.receptionist ! Receptionist.register(ActorY.key,ctx.self)
    new ActorFinding(ctx)
  }
}
